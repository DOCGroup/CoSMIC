#ifndef DLL_ENTRY_HPP
#define DLL_ENTRY_HPP

#include "extern_declarations.hpp"

// The caller should free the result with free
typedef char* (*DLLFunctionPtr) (const char*, size_t);

extern "C" OCMLConfigurator_API char*
DLLFunction(const char* values, size_t values_size);

/*
// The caller should free the *(char**) with delete[]
typedef void (*DLLFunctionPtr) (const char*, size_t, char**, size_t*);

extern "C" OCMLConfigurator_API void
DLLFunction(const char* values, size_t values_size,
            char** new_values, size_t* new_size);
*/

#endif // DLL_ENTRY_HPP
