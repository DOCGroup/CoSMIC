...
// MIC CLASS CODE BEGIN
    ...
    // Check here for Reliability QoS policies on the topic that would
    // apply to the data reader. This should only be checked if the
    // data reader didn't have a reliability policy explicitly specified.
    //
    // The same kind of code is needed for data writers as well.
    //
    // This kind of functionality is only needed for DBE since DBE doesn't
    // yet support QoS settings files for topics. Other interpreters that
    // supported getting QoS settings from topics wouldn't have this
    // problem either.
    //
    // Need to do something smarter here to check that there was no
    // reliability policy explicitly specified for the data reader.
		//if (!file_opened)
		//{
		//  outputDDSEntityQos (dataReader,
		//					            dr_name,
		//					            dr_prefix,
		//					            "top_reliability_Connection",
		//					            "Reliability",
		//					            attrib_map,
		//					            dr_count,
		//					            file_opened,
