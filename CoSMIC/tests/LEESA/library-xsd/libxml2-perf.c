 ...
 * author:      Modified by Sumant Tambe
...
void print_time_difference(struct timeval start, struct timeval end, const char *message);
...
execute_xpath_expression(const char* filename, const xmlChar* xpathExpr, const xmlChar* nsList) {
    ...
    gettimeofday(&end, 0);
    print_time_difference(start, end, "Parsing time = ");
    ...
    gettimeofday(&end, 0);
    print_time_difference(start, end, "Query time = ");
...
void print_time_difference(struct timeval start, struct timeval end, const char *message)
{
  ...
  retval.tv_sec = sec;
  retval.tv_usec = usec;
  fprintf(stdout, "%s [%u,%u]\n", message, (unsigned) retval.tv_sec, (unsigned) retval.tv_usec);
}
