...
namespace PICML
{
  ...
  {
    using namespace LEESA;
	evaluate(ia, ImplementationArtifacts() >>= 
		ArtifactContainer() >>= ImplementationArtifact() >> *this);
  ...
  {
	/*using namespace LEESA;
	//evaluate(ac, ArtifactContainer() >>= ImplementationArtifact() >> *this);
  ...
  void DeploymentPlanVisitor::generate_infoproperties (const DeploymentPlan &plan)
  {
    ...
    if (!info_properties.empty ())
    {
	  ...
	  using namespace LEESA;
	  evaluate(info_properties, Property() >> *this);
    ...
    const ImplementationArtifact& ia)
  {
    ...
    this->curr_ = ele;
	using namespace LEESA;
	evaluate(ia, ImplementationArtifact() 
		>> Association(ImplementationArtifact::dstArtifactExecParameter)
		>> *this);
  ...
  {
    using namespace LEESA;
	evaluate(cimpls, ComponentImplementations() 
		>>= ComponentImplementationContainer()
		>>= ComponentImplementation()
		>> CastFromTo(ComponentImplementation(), MonolithicImplementation())
		>> *this);
  ...
  {
	/*using namespace LEESA;
	//evaluate(cic, ComponentImplementationContainer() 
	//	>>= ComponentImplementation()
    //  >> CastFromTo(ComponentImplementation(), MonolithicImplementation())
	//	>> *this);
    ...
    (const MonolithicImplementation& mimpl)
  {
	...
	using namespace LEESA;
	evaluate(mimpl, MonolithicImplementation()
		>>& MonolithicImplementation::dstMonolithprimaryArtifact
		>> *this);
	...
	evaluate(mimpl, MonolithicImplementation() 
		>>& MonolithicImplementation::dstMonolithExecParameter
		>> *this);
  ...
  {
	/*//using namespace LEESA;
	//evaluate(dps, DeploymentPlans() >>= DeploymentPlan() >> *this);
  ...
  generate_implementation_descriptions (const PICML::DeploymentPlan & plan)
  {
	...
	using namespace LEESA;
	evaluate(root_folder_, RootFolder() >>= ComponentImplementations() >> *this);
  ...
  generate_artifact_descriptions (const PICML::DeploymentPlan & plan)
  {
	...
	using namespace LEESA;
	evaluate(root_folder_, RootFolder() >>= ImplementationArtifacts() >> *this);
  ...
  void DeploymentPlanVisitor::create_assembly_config_properties (Component& comp)
  {
	...
	evaluate(comp, Component() 
		>>& Component::dstAssemblyConfigProperty >> *this);
  ...
  void DeploymentPlanVisitor::create_component_config_properties 
	  (MonolithicImplementation& mimpl)
  {
    using namespace LEESA;
	evaluate(mimpl, MonolithicImplementation() 
		>>& MonolithicImplementation::dstConfigProperty >> *this);
  ...
  {
    using namespace LEESA;
	evaluate(comp, Component() >>= ReadonlyAttribute() >> *this);
    ...
    (const ComponentAssembly& assembly)
  {
	...
	using namespace LEESA;
	evaluate(assembly, ComponentAssembly() >> 
		MembersOf(ComponentAssembly(), invoke()           >> *this AND
		                               emit()             >> *this AND
				         			   publish()          >> *this AND
							           deliverTo()        >> *this AND
  ...
  void DeploymentPlanVisitor::update_connections(const ComponentAssembly& assembly)
  {
      ...
      ++iter)
    {
	  ...
	  using namespace LEESA;
		evaluate(subasm, ComponentAssembly() >> 
		MembersOf(ComponentAssembly(), invoke()           >> *this AND
		                               emit()             >> *this AND
				         			   publish()          >> *this AND
							           deliverTo()        >> *this AND
...
/*    If DP interpreter does not work as expected, uncomment this first.
  ...
  {
    using namespace LEESA;
	evaluate(assemblies, ComponentAssembly() >>= AttributeMapping() >> *this);
    /*for (std::vector<ComponentAssembly>::iterator iter = assemblies.begin();
      iter != assemblies.end();
      ++iter)
    {
