...
#include "CIDL_Export.h"
{
  ...
  {
  public:
    Cidlc_Visitor (const std::string& outputPath);
    ~Cidlc_Visitor();

	// Visit File
	virtual void Visit_File(const File&);
	virtual void Visit_ComponentFactory (const ComponentFactory &factory);
	virtual void Visit_ComponentRef(const ComponentRef&);
	virtual void Visit_Component(const Component&);
	virtual void process_Factory_decl (std::set<PICML::ComponentFactory> &factory);
	...
	// ComponentAssembly Implementation
