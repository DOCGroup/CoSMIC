...
/*
   ...
   0.	The p_backend points to an already open backend, and the framework
        ...
        To save changes to a different file use p_backend->SaveAs() or
        p_backend->CloseAs().
   ...
   3. The framework catches all the exceptions and reports the error in a
      message box, clean up and close the transactions aborting the
      changes. You can override this behavior by catching udm_exception.
      Use udm_exception::what() to form an error message.
*/
  ...
  try
    {
      ...
      AfxMessageBox("Your UDM interpreter setup is correct");
    }
