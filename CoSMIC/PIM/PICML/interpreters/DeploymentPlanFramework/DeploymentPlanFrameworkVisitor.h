// $Id: DeploymentPlanFrameworkVisitor.h,v 1.1.2.10 2006/04/02 21:00:57 jai Exp $
...
namespace PICML
{
  ...
  private:
    ...
    std::multimap<std::string, InEventPort> consumers_;
	  ...
	  std::map<std::string, PICML::Component> instance_monolith_components_;
	  std::map<std::string, PICML::Component> instance_assembly_components_;
	  std::map<std::string, Injector*> injectors_;
    std::map < std::string, std::pair <PICML::InstanceConnection, PICML::InstanceConnection> > connections_;
  };
}
