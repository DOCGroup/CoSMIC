...
namespace PICML
{
  ...
  std::map<std::string, PICML::Component> FTInjector::add_monolith_instances ()
    {
      return this->comp_addr_->get_all_monolith_components ();
    }

  std::map<std::string, PICML::Component> FTInjector::add_assembly_instances ()
    {
