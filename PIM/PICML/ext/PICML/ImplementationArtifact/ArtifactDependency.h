// -*- C++ -*-

//============================================================================
/**
 * @file    ArtifactDependency.h
 *
 * $Id$
 *
 * @author  Alhad Mokashi <amokashi at iupui dot edu>
 *          James H. Hill <hillj at cs dot iupui dot edu>
 */
//============================================================================

#ifndef _PICML_IMPLEMENTATIONARTIFACT_ARTIFACTDEPENDENCY_H_
#define _PICML_IMPLEMENTATIONARTIFACT_ARTIFACTDEPENDENCY_H_

#include "PICML/PICML_fwd.h"
#include "PICML/PICML_export.h"

#include "game/mga/Connection.h"

namespace PICML
{
  // Forward decl. and type definitions
  class ArtifactDependency_Impl;
  typedef ArtifactDependency_Impl * ArtifactDependency_in;
  typedef ::GAME::Mga::Smart_Ptr < ArtifactDependency_Impl > ArtifactDependency;

  // Forward decl.
  class Visitor;

  /**
   * @class ArtifactDependency_Impl
   *
   * Implementation for the ArtifactDependency model element.
   */
  class PICML_Export ArtifactDependency_Impl :
    public virtual ::GAME::Mga::Connection_Impl
  {
    public:
    /// Tag type of this extension class.
    typedef ::GAME::Mga::connection_tag_t type_tag;

    /// Type definition of this class's interface.
    typedef IMgaConnection interface_type;

    /// Metaname for this extension class.
    static const std::string metaname;

    /// Identifier if this class is an abstract type in GME
    static const bool is_abstract;

    /**
     * @name Factory Methods
     */
    ///@{
    static ArtifactDependency _create (const ArtifactContainer_in parent, ImplementationArtifact_in src, ImplementationArtifact_in dst);
    ///@}

    // Default constructor.
    ArtifactDependency_Impl (void);

    // Initializing constructor.
    ArtifactDependency_Impl (IMgaConnection * ptr);

    // Destructor.
    virtual ~ArtifactDependency_Impl (void);

    /// Accept a visitor for this model element.
    virtual void accept (::GAME::Mga::Visitor * v);

    /**
     * @name Parent Methods
     */
    ///@{
    ArtifactContainer parent_ArtifactContainer (void);
    ///@}
    ImplementationArtifact src_ImplementationArtifact (void) const;
    ImplementationArtifact dst_ImplementationArtifact (void) const;
  };
}

#if defined (__GAME_INLINE__)
#include "ArtifactDependency.inl"
#endif

#endif  // !defined _PICML_IMPLEMENTATIONARTIFACT_ARTIFACTDEPENDENCY
