...
public:
  ...
  void create_workspace (const PICML::Workspaces_in workspace);
  void write_mwc_footer ();
  void write_mwc_header ();

  // Project related definitions
  void create_project (const PICML::Project_in project);
  void create_stub_definition (const PICML::StubProject_in );
  void create_skeleton_definition (const PICML::ServantProject_in , bool);
  void create_executor_definition (const PICML::ExecutorProject_in);
  bool dependent_project (PICML::ImplementationArtifact_in artifact,
  ...
  void create_cidl_defn (PICML::ImplementationArtifact_in artifact);
  void create_export_macro (std::string & shared_name);
  std::string skel_export_macro (PICML::Project_in project);
  std::vector<std::string> generate_dependant_libs (std::vector <PICML::ArtifactDependsOn> art_refs);
