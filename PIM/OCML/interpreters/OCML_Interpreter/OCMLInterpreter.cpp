...
OCMLInterpreter::create_option_tree(std::set<OCML_BON::Option_Category>& categories)
{
  ...
  xercesc::DOMDocument* doc =
    impl_->createDocument(0, XStr("OCML_Categories"), 0);

  // Traverse the root models and ...
  for (std::set<OCML_BON::Option_Category>::iterator iter = categories.begin();
       iter != categories.end(); ++iter)
  {
    // call xml_export functions of each Option_Category
    // and insert the returning node into the xml node
    xercesc::DOMNode* child = (*iter)->xml_export(doc);
